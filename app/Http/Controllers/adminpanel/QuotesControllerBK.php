...
class QuotesControllerBK extends Controller
{
    ...
    public function save_quote_edit($id,Request $request){
       ...
       foreach($request['product_details'] as $key=>$productData){
        ...
        $total_products=count($productData['cat_id']);
            for($i=0; $i<$total_products; $i++){
            if(isset($productData['product_name'][$i]) && $productData['product_name'][$i]!=''){
    ...
    public function save_quote_data(Request $request){
       ...
       foreach($request['product_details'] as $key=>$productData){
        ...
        if(isset($productData['product_name']) && count($productData['product_name'])>0){
            ...
            echo 'cat_id :'.count($productData['cat_id']).'<br>';
            $total_products=count($productData['cat_id']);
            for($i=0; $i<$total_products; $i++){
